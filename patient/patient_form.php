...
<link rel="stylesheet" href="styles.css">
  ...
  <body class="u-body"><header class="u-clearfix u-header u-header" id="sec-d9f7"><div class="u-clearfix u-sheet u-sheet-1">
      ...
      $mode = "입력";
      $action = "patient_insert.php";

      if (array_key_exists("pat_no", $_GET)) {
          $pat_no = $_GET["pat_no"];
          $query =  "select * from patient where pat_no = $pat_no";
          ...
          $mode = "수정";
          $action = "patient_modify.php";

          //echo json_encode($pat);
      }
      ?>
          <div class="container">
              <form name="patient_form" action="<?=$action?>" method="post" class="fullwidth">
                  <input type="hidden" name="pat_no" value="<?=$pat['pat_no']?>"/>
                  <h3>환자 정보 <?php echo $mode; ?></h3>
                  <p>
                  ...
                  <p>
                      <label for="pat_phone">환자전화번호</label>
                      <input type="text" placeholder="환자전화번호 입력" name="pat_phone" value="<?=$pat['pat_phone']?>"/>
                  </p>
                  ...
                  <script>
                      function validate() {
                          ...
                          else if(document.getElementById("pat_phone").value == "") {
                              alert ("환자의 전화번호를 입력해 주십시오"); return false;
                          }
                          return true;
                      }
                  </script>
