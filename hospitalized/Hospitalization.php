...
<link rel="stylesheet" href="styles.css">
  ...
  <body class="u-body"><header class="u-clearfix u-header u-header" id="sec-d9f7"><div class="u-clearfix u-sheet u-sheet-1">
      ...
      <div class="container">
          ...
          $query = "select * from hospitalized natural join bedinfo";
          ...
          <table class="table table-striped table-bordered">
              ...
              while ($row = mysqli_fetch_array($res)) {
                  ...
                  echo "<td>{$row['hosp_date']}</td>";
                  echo "<td>{$row['bed_no']}</td>";
                  echo "<td>{$row['room_no']}</td>";
                  echo "<td>{$row['room_call']}</td>";
                  echo "<td width='17%'>
                      <a href='hosp_form.php?pat_no={$row['pat_no']}'><button class='button primary small'>수정</button></a>
          ...
          <center><div><h4><a href = 'hosp_form.php'>입원환자 등록</a></h4></div></center>
          <script>
              function deleteConfirm(pat_no) {
                  if (confirm("정말 삭제하시겠습니까?") == true){    //확인
                      window.location = "hosp_delete.php?pat_no=" + pat_no;
                  }else{   //취소
                      return;
                  }
              }
