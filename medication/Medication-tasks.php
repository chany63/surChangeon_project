...
<link rel="stylesheet" href="styles.css">
  ...
  <body class="u-body"><header class="u-clearfix u-header u-header" id="sec-d9f7"><div class="u-clearfix u-sheet u-sheet-1">
      ...
      <div class="container">
          ...
          $query = "select * from meditask";
          if (array_key_exists("search_keyword", $_POST)) {  // array_key_exists() : Checks if the specified key exists in the array
              $search_keyword = $_POST["search_keyword"];
              $query =  $query . " where emp_no like '%$search_keyword%'";
          ...
          <table class="table table-striped table-bordered">
              ...
              while ($row = mysqli_fetch_array($res)) {
                  ...
                  echo "<td>{$row['medi_date']}</td>";
                  echo "<td width='17%'>
                      <a href='medi_form.php?task_no={$row['task_no']}'><button class='button primary small'>수정</button></a>
                       <button onclick='javascript:deleteConfirm({$row['task_no']})' class='button danger small'>삭제</button>
                      </td>";
                  echo "</tr>";
                  $row_index++;
              }
          ...
          <center><div><h4><a href = 'medi_form.php'>치료업무 등록</a></h4></div></center>
          <script>
              function deleteConfirm(task_no) {
                  if (confirm("정말 삭제하시겠습니까?") == true){    //확인
                      window.location = "medi_delete.php?task_no=" + task_no;
                  }else{   //취소
                      return;
                  }
              }
