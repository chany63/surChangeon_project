...
<link rel="stylesheet" href="Hospitalization.css" media="screen">
<link rel="stylesheet" href="styles.css">
  ...
  <body class="u-body"><header class="u-clearfix u-header u-header" id="sec-d9f7"><div class="u-clearfix u-sheet u-sheet-1">
      ...
      $mode = "입력";
      $action = "hosp_insert.php";

      if (array_key_exists("pat_no", $_GET)) {
          $pat_no = $_GET["pat_no"];
          $query =  "select * from hospitalized where pat_no = $pat_no";
          $res = mysqli_query($conn, $query);
          $pat = mysqli_fetch_array($res);
          if(!$pat) {
              msg("입원환자가 존재하지 않습니다.");
          }
          $mode = "수정";
          $action = "hosp_modify.php";
          //echo json_encode($pat);
      }
      ?>
          <div class="container">
              <form name="hosp_form" action="<?=$action?>" method="post" class="fullwidth">
                  <input type="hidden" name="pat_no" value="<?=$pat['pat_no']?>"/>
                  <h3>입원환자 정보 <?php echo $mode; ?></h3>
                  <p>
                      <label for="pat_no">환자번호</label>
                      <input type="text" placeholder="환자번호 입력" name="pat_no" value="<?=$pat['pat_no']?>"/>
                  </p>
                  <p>
                      <label for="pat_name">환자이름</label>
                      <input type="text" placeholder="환자이름 입력" name="pat_name" value="<?=$pat['pat_name']?>"/>
                  </p>
                  <p>
                      <label for="hosp_date">입원날짜</label>
                      <input type="text" placeholder="입원날짜 입력(ex: 211120)" name="hosp_date" value="<?=$pat['hosp_date']?>"/>
                  </p>
                  <p>
                      <label for="bed_no">침대번호</label>
                      <input type="text" placeholder="침대번호 입력" name="bed_no" value="<?=$pat['bed_no']?>" />
                  </p>
                  ...
                  <script>
                      function validate() {
                          if(document.getElementById("pat_no").value == "-1") {
                              alert ("환자의 할당번호를 입력해 주십시오"); return false;
                          }
                          else if(document.getElementById("pat_name").value == "") {
                              alert ("환자의 이름을 입력해 주십시오"); return false;
                          }
                          else if(document.getElementById("hosp_date").value == "") {
                              alert ("환자의 입원날짜를 입력해 주십시오"); return false;
                          }
                          else if(document.getElementById("bed_no").value == "") {
                              alert ("환자가 입원할 침대번호를 입력해 주십시오"); return false;
                          }
                          return true;
                      }
                  </script>
